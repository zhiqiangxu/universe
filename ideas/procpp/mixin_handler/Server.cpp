...
bool Server::listen(const struct sockaddr *addr, socklen_t addrlen, EventManager::EventCB callbacks, int type)
{
    ...
    const int enable = 1;
    if (setsockopt(s, SOL_SOCKET, SO_REUSEADDR/* ignores TIME-WAIT state, http://serverfault.com/a/329846 */, &enable, sizeof(enable)) == -1) L.error_exit("setsockopt");
...
int Server::accept(int socketfd, struct sockaddr *addr, socklen_t *addrlen)
{
    ...
    cout << "[accept] ok client = " + to_string(client) + " pid=" + to_string(Utils::getpid()) << endl;
