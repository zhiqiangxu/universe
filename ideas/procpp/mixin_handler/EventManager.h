...
public:
    ...
    //目前只是对可写事件做处理
    virtual bool auto_rewatch(int fd, bool et) = 0;
    ...
    ****/
    ...
    virtual string read(int fd) = 0;//fd为ET
    virtual ssize_t read(int fd, void* buf, size_t size) = 0;
    virtual string read_until(int fd, string separator) = 0;
...
public:
    ...
    virtual bool auto_rewatch(int fd, bool et = false) override;
    //TODO finish reading:
    ...
    virtual ssize_t read(int fd, void* buf, size_t size) override;
    virtual string read_until(int fd, string separator) override;
...
protected:
    void _append_write_buf(int fd, const void *buf, size_t count);
    bool _has_write_buf(int fd);
    bool _epoll_update(int fd, int epoll_op, bool et = true);
    void _add_close_fd(int fd);
    bool _finish_write_buf(int fd);
    ...
    map<int, SSLConnection*> _fd_ssl;
    map<int, string> _write_buf;
