...
{
    return connect(address, port, callbacks, async, false);
}

int Client::connect_ssl(string address, uint16_t port, EventManager::EventCB callbacks, bool async)
{
    return connect(address, port, callbacks, async, true);
}

int Client::connect(string address, uint16_t port, EventManager::EventCB callbacks, bool async, bool ssl)
{
    //domain name
    struct addrinfo hints;
    ...
    if (ssl) {
        //TODO move to EventManager
        if (_fd_ssl.find(s) != _fd_ssl.end()) {
            delete _fd_ssl[s];
        }
        auto p_ssl_conn = new SSLConnection(s, true);
        if (!p_ssl_conn->initialize()) {
            L.error_log("SSLConnection::initialize failed");
            delete p_ssl_conn;
            close(s, true);
            callbacks[EventType::CONNECT](-1, ConnectResult::NG);
            return -1;
        } else {
            L.debug_log("SSLConnection::initialize ok");
        }
        _fd_ssl[s] = p_ssl_conn;
    }
