...
void StreamReader::read_size(size_t size, void* p)
{
    ...
    if (p) memcpy(p, _str.data() + _offset, size);
    _offset += size;
}

void StreamReader::read_size(size_t size, string& result)
{
    if (size > _str.length() - _offset) throw ReaderException::AG;

    result.append(_str.data() + _offset, size);
    _offset += size;
}

void StreamReader::read_size_offset(size_t size, void* p, size_t offset)
{
    if (size > _str.length() - offset) throw ReaderException::AG;

    memcpy(p, _str.data() + offset, size);
}
